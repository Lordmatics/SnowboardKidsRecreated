...
ASnowboardCharacterBase* USnowboardCharacterSubsystem::FindClosestCharacterWithinRange(const ASnowboardCharacterBase& Shooter, const AActor& Projectile, float Range) const
{
	...
	for (ASnowboardCharacterBase* CurrentCharacter : RegisterredCharacters)
	{
		...
		const float RangeSquared = Range;// FMath::Square<float>(Range);
